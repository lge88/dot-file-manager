...
exports.findDotFileInRepo = function(file) {
  var relPath = path.relative(home, file);
  if (isSubDirOf(file, home)) {
    if (exists(join(repoPath, relPath))) {
      return join(repoPath, relPath);
    }
  }
  return false;
};
